...
## 빠른 시작 (다른 시스템에서 clone — 의존성만 있으면 즉시 동작)

```bash
git clone https://github.com/mrgrit/6bq5.git
cd 6bq5

# 0. 의존성 (Python 3.10+, Node 18+)
pip install -r backend/requirements.txt
# (선택) frontend dev 모드 쓸 거면: cd frontend && npm install

# 1. 부팅 — KG 는 이미 sanitized snapshot 으로 repo 안에 있음
./run.sh
#   → http://0.0.0.0:8500/   (UI + API 통합)
```

`run.sh` 가 하는 일:
1. `data/kg.db` (precinct6-제거된 3,802노드 KG, 6.5MB) 존재 확인
2. `frontend/dist` 없으면 `npm install && npm run build`
3. `uvicorn backend.main:app` 부팅
