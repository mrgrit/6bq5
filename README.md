...
## precinct6 데이터 처리 (v0.1.1 강화)

`scripts/import_kg.py` 가 다음을 자동 제외:
- `id` 가 `asset:p6:` 로 시작하는 노드
- `id` 가 `asset:100.64.*` 로 시작하는 노드 (precinct6 sample subnet)
- 엣지가 0 인 `asset:172.*` 노드 (precinct6 random subnet 잔재)
- `meta` 또는 **`content`** JSON 안에 `precinct6` 가 들어간 노드
- `history_anchors` 의 `kind='ioc'` 행 (precinct6 IoC dump)
- `history_anchors` 본문/related_ids 가 100.64.* 또는 precinct6 를 언급한 행
- 위에 매달려 있던 dangling edge

> v0.1.0 에서는 `meta`/`id` 만 검사해 4,879 개 precinct6 노드 (content 표지) 가
> 누수되었음. v0.1.1 에서 완전 제거. 결과: **5,274 → 436 노드** (CCC 운영 컨텐츠
> 만 보존).
